...
public class DirectorWindow : EditorWindow
{
    ...
    private void updateToolbar(Rect toolbarArea)
    {
        ...
        if (GUILayout.Button(CREATE, EditorStyles.toolbarDropDown, GUILayout.Width(60)))
        {
            ...
            if (cutscene != null)
            {
                ...
                createMenu.AddItem(new GUIContent("Director Group"), false, addDirectorGroup);
                createMenu.AddItem(new GUIContent("Actor Group"), false, addActorGroup);
                createMenu.AddItem(new GUIContent("Multi Actor Group"), false, addMultiActorGroup);
            }
    ...
    /// <summary>
    /// Add an empty actor group to the current cutscene.
    /// </summary>
    private void addActorGroup()
    {
        Undo.RegisterCreatedObjectUndo(CutsceneItemFactory.CreateActorTrackGroup(cutscene).gameObject, "Create Actor Group");
    }
